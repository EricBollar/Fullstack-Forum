...
class UnitOfWork {
    ...
    expandUniqueProps(entity) {
        return (0, entity_1.helper)(entity).__meta.uniqueProps.map(prop => {
            if (entity[prop.name]) {
    ...
    postCommitCleanup() {
        this.changeSets.forEach(cs => (0, entity_1.helper)(cs.entity).__processing = false);
    ...
    fixMissingReference(entity, prop) {
        ...
        if ([enums_1.ReferenceType.MANY_TO_ONE, enums_1.ReferenceType.ONE_TO_ONE].includes(prop.reference) && reference && !prop.mapToPk) {
            if (!Utils_1.Utils.isEntity(reference)) {
                ...
                const pk = (0, entity_1.helper)(reference).getSerializedPrimaryKey();
