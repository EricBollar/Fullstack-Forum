...
class Collection extends ArrayCollection_1.ArrayCollection {
    ...
    async matching(options) {
        ...
        if (this.property.reference === enums_1.ReferenceType.MANY_TO_MANY && em.getPlatform().usesPivotTable()) {
            const map = await em.getDriver().loadFromPivotTable(this.property, [(0, wrap_1.helper)(this.owner).__primaryKeys], where, opts.orderBy, ctx, options);
    ...
    async init(options = {}) {
        ...
        if (!this.initialized && this.property.reference === enums_1.ReferenceType.MANY_TO_MANY && em.getPlatform().usesPivotTable()) {
            const map = await em.getDriver().loadFromPivotTable(this.property, [(0, wrap_1.helper)(this.owner).__primaryKeys], options.where, options.orderBy, undefined, options);
    ...
    checkInitialized() {
        if (!this.isInitialized()) {
            throw new Error(`Collection<${this.property.type}> of entity ${this.owner.constructor.name}[${(0, wrap_1.helper)(this.owner).getPrimaryKey()}] not initialized`);
        }
    }
    /**
