...
class MetadataDiscovery {
    ...
    async discoverMissingTargets() {
        ...
        for (const prop of missing) {
            const target = (prop.entity instanceof Function ? prop.entity() : prop.type);
    ...
    async saveToCache(meta) {
        ...
        const copy = Utils_1.Utils.copy(meta);
    ...
    async defineFixedOrderProperty(prop, targetType) {
    ...
    initCustomType(meta, prop) {
        ...
        if (!prop.customType && !prop.columnTypes && ['string[]', 'array'].includes(prop.type)) {
            prop.customType = types_1.Type.getType(types_1.ArrayType);
        }
        ...
        if (!prop.customType && !prop.columnTypes && prop.type === 'Buffer') {
            prop.customType = types_1.Type.getType(types_1.BlobType);
        }
        if (!prop.customType && !prop.columnTypes && prop.type === 'json') {
            prop.customType = types_1.Type.getType(types_1.JsonType);
        }
        // `prop.type` might be actually instance of custom type class
        if (types_1.Type.isMappedType(prop.type)) {
        ...
        if (types_1.Type.isMappedType(prop.customType) && prop.reference === enums_1.ReferenceType.SCALAR) {
    ...
    async initColumnType(prop, path) {
    ...
    async initEnumValues(prop, path) {
